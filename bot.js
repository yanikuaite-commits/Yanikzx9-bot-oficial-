...
};// ─── FIM DO OBJETO COMMANDS ──────────────────────────────────────────
// (o "};" que fecha o objeto commands)
};
...
async function executarAntiLink(sock, chatId, msg, senderId, modo) {
  ...
  if (modo === 'warn') {
    await sock.sendMessage(chatId, { text: `⚠️ @${senderId.split('@')[0]}, links não são permitidos!`, mentions: [senderId] });
  } else if (modo === 'delete') {
    await sock.sendMessage(chatId, { text: `🔗 Link removido — @${senderId.split('@')[0]}`, mentions: [senderId] });
  } else if (modo === 'kick' || modo === 'ban') {
    try {
      await sock.groupParticipantsUpdate(chatId, [senderId], 'remove');
      registrarAcao(chatId, `Anti-link (${modo}): @${senderId.split('@')[0]}`);
      await sock.sendMessage(chatId, { text: `🚫 @${senderId.split('@')[0]} removido por enviar link.`, mentions: [senderId] });
    } catch {
...
rlTerminal.on('line', (linha) => {
  ...
  if (cmd === 'parar' || cmd === '.parar') {
    geracaoAtual++;
    pausado = true;
    console.log('🛑 PARADO — mensagens canceladas. Digita "continuar" para retomar.');
...
// =================== LIMPEZA PERIÓDICA DE HISTÓRICO IA ===================
setInterval(() => {
  const agora = Date.now();
  const LIMITE_INATIVIDADE = 30 * 60 * 1000;
  for (const [chatId, ultimoUso] of db.historicoIAUltimoUso) {
    if (agora - ultimoUso > LIMITE_INATIVIDADE) {
  ...
  try {
    ...
    setInterval(async () => {
      try {
        ...
        for (const l of prontos) {
          try { await sock.sendMessage(`${CONFIG.ownerId}@s.whatsapp.net`, { text: `⏰ *LEMBRETE*\n\n📝 ${l.texto}` }); } catch (e) { console.error('Erro lembrete:', e.message); }
        }
      } catch (e) { console.error('Erro verificador lembretes:', e.message); }
    }, 60000);
      ...
      if (action === 'add') {
        ...
        if (participants.includes(botJid)) {
          if (!utils.isGroupSubscribed(groupId)) {
            await sock.sendMessage(groupId, { text: `⚠️ Este grupo não possui assinatura activa.\nContacte ${CONFIG.creator}: ${CONFIG.ownerNumber}` });
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        // ── PV (não é dono) → cartão de apresentação ──
        ...
        if (isGroup && fullText) {
          ...
          if (dadosTransacoes?.ativo) {
            ...
            if (comprovativo) {
              ...
              if (contaValida) {
                ...
                } catch (e) { console.warn('Erro notificar admins:', e.message); }
                return;
              }
            }
          }

          // ── Anti-link e palavras banidas (apenas não-admins) ──
          if (!isAdmin && !isOwner) {
            ...
            for (const palavra of palavrasBanidas) {
              if (fullText.toLowerCase().includes(palavra)) {
                try { await sock.sendMessage(chatId, { delete: msg.key }); } catch {}
                await sock.sendMessage(chatId, { text: `🚫 Palavra proibida — @${senderId.split('@')[0]}`, mentions: [senderId] });
                return;
              }
            }
          }
          ...
          if (db.grupos.audioGatilhos.has(chatId)) {
            const gatilhos = db.grupos.audioGatilhos.get(chatId);
            const textoLower = fullText.toLowerCase().trim();
            if (gatilhos.has(textoLower)) {
              const audioBuf = gatilhos.get(textoLower);
              await sock.sendMessage(chatId, { audio: audioBuf, mimetype: 'audio/mpeg', ptt: true });
              return;
            }
          }
          ...
          if (db.grupos.midiaGatilhos.has(chatId)) {
            const gatilhos = db.grupos.midiaGatilhos.get(chatId);
            const textoLower = fullText.toLowerCase().trim();
            if (gatilhos.has(textoLower)) {
              const { buffer, tipo } = gatilhos.get(textoLower);
              if (tipo === 'image') await sock.sendMessage(chatId, { image: buffer });
              else if (tipo === 'video') await sock.sendMessage(chatId, { video: buffer, mimetype: 'video/mp4' });
              else await sock.sendMessage(chatId, { document: buffer, mimetype: 'application/octet-stream' });
              return;
            }
          }

          // ── Mensagens sem prefixo (atalhos, IA, saudações) ──
          if (!fullText.startsWith(CONFIG.prefix)) {
            ...
            if (textoLower.includes('nano') || textoLower.includes('bot') || textoLower.includes('@' + CONFIG.botNumber)) {
              ...
              if (utils.isOwner(senderId) && isGroup && pareceIntentoSairGrupo(fullText)) {
                await sock.sendMessage(chatId, { text: '👋 Ok, já saio!' });
        ...
        if (fullText?.startsWith(CONFIG.prefix)) {
          ...
          if (cmd && commands[cmd]) {
            // Verificar se comando está desativado neste grupo
            if (isGroup && db.grupos.comandosDesativados.has(chatId)) {
              const desativados = db.grupos.comandosDesativados.get(chatId);
              if (desativados.has(cmd) && !utils.isOwner(senderId)) {
                return await sock.sendMessage(chatId, { text: `🚫 O comando .${cmd} está desativado neste grupo.` });
              }
            }

            // Verificar mídia associada ao comando
            if (isGroup && db.grupos.midiaComandos.has(chatId)) {
              const midias = db.grupos.midiaComandos.get(chatId);
              const chave = `.${cmd}`;
              if (midias.has(chave)) {
                const { buffer, tipo } = midias.get(chave);
                if (tipo === 'image') await sock.sendMessage(chatId, { image: buffer }).catch(() => {});
                else if (tipo === 'video') await sock.sendMessage(chatId, { video: buffer, mimetype: 'video/mp4' }).catch(() => {});
              }
            }
            ...
            if (!rl.permitido) {
              ...
              const tempo = seg > 60 ? `${Math.ceil(seg / 60)} min` : `${seg}s`;
              return await sock.sendMessage(chatId, { text: `⏳ Calma! Já usaste .${cmd} ${RATE_LIMIT_MAX}x. Aguarda ${tempo}` });
            }
...
if (!process.env.GROQ_API_KEY) {
  console.warn('⚠️ GROQ_API_KEY não definida — IA pode falhar.');
}
