...
// Reduz footprint de memória do sharp (hosts com pouca RAM)
sharp.cache(false);
sharp.concurrency(1);
...
const CONFIG = {
  botName: "Nano Bot 🤖",
  creator: "Yanik Uaite",
  ownerId: "275381038891241", // ✅ LID real do dono (confirmado nos logs)
...
// Cola a tua chave entre as aspas abaixo (começa por gsk_...)
// Se um dia puseres a chave no .env/Runny também, o .env tem prioridade.
const GROQ_API_KEY_DIRETA = "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";
...
const RANK_VIP = { ouro: 1, diamante: 2, lenda: 3 };// =================== PERSISTÊNCIA DO BOT ===================
...
const utils = {
  ...
  getGrupoContexto: (groupId) => {
    ...
    return `\n📌 Assinatura: ${NIVEIS_VIP[sub.nivel].nome} (${d}d ${h}h restantes)`;
  },
  tempoRestante: (ms) => {
...
// ✅ FIX DONO: resolve LID do dono usando metadata do grupo
...
const commands = {
  ...
  // ─── DOWNLOADS (TikTok / Instagram / YouTube via GenDownload) ──────────
  'tk': async (sock, ctx) => {
...
rlTerminal.on('line', (linha) => {
  ...
  if (cmd === 'parar' || cmd === '.parar') {
    ...
    console.log(`🔄 A reiniciar a ligação — ${ultimasMensagensIds.length} mensagens serão ignoradas.`);
    try { sockAtual?.end(new Error('Reinício manual')); } catch (e) {}
