...
const CONFIG = {
  botName: "Nano Bot 🤖",
  creator: "Yanik Uaite",
  ownerId: "27538103889241", // ✅ LID real do dono (confirmado nos logs)
  ownerNumber: "834788141",
  botNumber: "258850421617", // ✅ número actual do bot
  prefix: ".",
  dataFile: path.join(__dirname, 'data', 'bot_data.json'),
  historicoFile: path.join(__dirname, 'data', 'historico.json')
};

// 🔑 CHAVE GROQ DIRECTAMENTE NO CÓDIGO
// Substitui 'COLE_AQUI_A_TUA_CHAVE' pela tua chave real (começa por gsk_...)
const groq = new Groq({ 
...
const db = {
  ...
  grupoDono: new Map(),
  historicoIA: new Map(),
  historicoIAUltimoUso: new Map(), // ✅ limpeza de históricos inativos (RAM)
  statusDono: null,
  historicoGrupos: new Map(),
  atalhos: new Map(),
  lembretes: [],
  ultimoCartaoPV: new Map(),
  usersVIP: new Map(), // ✅ VIP de utilizadores individuais (senderId -> {nivel, expiraEm})
  grupos: {
    antiLink: new Map(),
    ...
    transacoes: new Map(),
    desligados: new Set(),
    comandosDesativados: new Map(), // ✅ groupId -> Set de comandos desativados
    audioGatilhos: new Map(),       // ✅ groupId -> Map palavra -> audioBuffer
    midiaGatilhos: new Map(),       // ✅ groupId -> Map palavra -> {buffer, tipo}
    midiaComandos: new Map()        // ✅ groupId -> Map comando -> {buffer, tipo}
  },
  ignorados: new Set(),
  whitelist: new Map(),
  autoDelete: new Map(),
...
// ✅ Cache de groupMetadata (TTL 30s) — reduz pedidos HTTP e RAM
...
const REGEX_EMOLA = /id da transac[aã]o[:\s]+([A-Za-z0-9.]+)\s*[\.\s]\s*transferiste\s+(\d+(?:[.,]\d{1,2})?)\s*mt\s*para\s*conta\s*(\d+)(?:,\s*nome:\s*([^\n.]+?))?\s+as\s/i;
const REGEX_MPESA = /confirmado[:\s]+([A-Za-z0-9]+)\.\s*transferiste\s+(\d+(?:[.,]\d{1,2})?)\s*mt.*?para\s+(\d+)\s+aos\s/i;
...
// ✅ Hierarquia de VIP para UTILIZADORES (acesso a downloads)
...
function carregarDados() {
  try {
    ...
    if (fs.existsSync(CONFIG.dataFile)) {
      ...
      // ✅ Novos: usersVIP, comandosDesativados, audioGatilhos, midiaGatilhos, midiaComandos
...
// ✅ FIX DONO: resolve o LID do dono usando os metadados do grupo
...
// ✅ Verifica se o utilizador tem permissão para usar um comando de mídia (VIP de utilizador)
...
'ping': async (sock, ctx) => {
  ...
  'hora': async (sock, ctx) => {
    ...
    const h = parseInt(agora.toLocaleTimeString('pt-PT', { timeZone: 'Africa/Maputo', hour: '2-digit', hour12: false }));
    let periodo, emoji;
    if (h >= 5 && h < 12) { periodo = 'Bom dia! ☀️'; emoji = '🌅'; }
    else if (h >= 12 && h < 18) { periodo = 'Boa tarde! 🌤️'; emoji = '☀️'; }
    else if (h >= 18 && h < 21) { periodo = 'Boa noite! 🌆'; emoji = '🌇'; }
    else { periodo = 'Boa madrugada! 🌙'; emoji = '🌙'; }
    await sock.sendMessage(ctx.chatId, { text: `╭━━━━━━━━━━━━━━━━━━━━━━━╮\n┃ ${emoji} *${periodo}*\n┃\n┃ ⏰ São exatamente *${hora}*\n┃ 📅 ${data}\n┃ 🌍 Fuso: Maputo (CAT)\n┃\n┃ 💬 _"Cada segundo conta,_\n┃ _aproveita bem o teu dia!"_\n╰━━━━━━━━━━━━━━━━━━━━━━━╯` });
},
  'tid': async (sock, ctx) => {
  ...
  'auto': async (sock, ctx) => {
    ...
    const parse = (v) => { const m = v.match(/^(\d+)(s|m|h)?$/i); if (!m) return null; const n = parseInt(m[1],10); const u = (m[2]||'s').toLowerCase(); if (u==='s') return n*1000; if (u==='m') return n*60000; return n*3600000; };
    const ms = parse(mode);
  ...
  'indicar': async (sock, ctx) => {
    ...
    const cur = db.indicadores.get(ctx.senderId) || 0; db.indicadores.set(ctx.senderId, cur + 1); salvarDados();
  ...
  'grcb': async (sock, ctx) => {
    ...
    try { const buf = await sharp(Buffer.from(svg)).png().toBuffer(); await sock.sendMessage(ctx.chatId, { image: buf, caption: `🧾 Comprovativo — ${tema.nome}` }); } catch (e) { await sock.sendMessage(ctx.chatId, { text: 'Erro ao gerar comprovativo.' }); }
  },'t': async (sock, ctx) => {
  ...
  'tconta': async (sock, ctx) => {
    ...
    if (acao === 'add') { if (!numero) return sock.sendMessage(ctx.chatId, { text: 'Uso: .tconta add [número]' }); if (!dados.contas.includes(numero)) dados.contas.push(numero); salvarDados(); return sock.sendMessage(ctx.chatId, { text: `✅ Conta ${numero} adicionada.` }); }
    if (acao === 'rm' || acao === 'remove') { if (!numero) return sock.sendMessage(ctx.chatId, { text: 'Uso: .tconta rm [número]' }); dados.contas = dados.contas.filter(c => c !== numero); salvarDados(); return sock.sendMessage(ctx.chatId, { text: `️ Conta ${numero} removida.` }); }
  ...
  'prefix': async (sock, ctx) => {
    ...
    CONFIG.prefix = novoPrefixo; salvarDados();
  ...
  'ignorar': async (sock, ctx) => {
    ...
    // ✅ VIPs têm imunidade ao .ignorar
    if (db.usersVIP.has(target) && db.usersVIP.get(target).expiraEm > Date.now()) {
      return sock.sendMessage(ctx.chatId, { text: `❌ Não posso ignorar este utilizador, pois ele possui um plano VIP activo.`, mentions: [target] });
    }
  ...
  'ban': async (sock, ctx) => {
    ...
    try {
     ...
     await sock.sendMessage(ctx.chatId, { text: `╭━━━━━━━━━━━━━━━━━━━━━━━╮\n┃ 🔨 *BANIDO!*\n┃\n┃ 👤 @${target.split('@')[0]}\n┃ 📅 ${new Date().toLocaleDateString('pt-PT')}\n┃ 👮 Por: @${ctx.senderId.split('@')[0]}\n┃\n┃ ⚠️ _Quem não cumpre as regras,_\n┃ _não fica no grupo!_\n╰━━━━━━━━━━━━━━━━━━━━━━━╯`, mentions: [target, ctx.senderId] }); 
  ...
  'up': async (sock, ctx) => {
    ...
    let target = utils.getQuotedMention(ctx.msg) || utils.getMentions(ctx.msg)[0];
    if (!target) return sock.sendMessage(ctx.chatId, { text: `❌ Menciona alguém.` });
    try { await sock.groupParticipantsUpdate(ctx.chatId, [target], 'promote'); await sock.sendMessage(ctx.chatId, { text: `👑 @${target.split('@')[0]} promovido`, mentions: [target] }); } catch { await sock.sendMessage(ctx.chatId, { text: `❌ Erro ao promover.` }); }
  },
  'down': async (sock, ctx) => {
    if (!ctx.isGroup || !(await utils.hasPromoteRights(sock, ctx.chatId, ctx.senderId))) return;
    let target = utils.getQuotedMention(ctx.msg) || utils.getMentions(ctx.msg)[0];
    if (!target) return sock.sendMessage(ctx.chatId, { text: `❌ Menciona alguém.` });
    try { await sock.groupParticipantsUpdate(ctx.chatId, [target], 'demote'); await sock.sendMessage(ctx.chatId, { text: `⬇️ @${target.split('@')[0]} rebaixado`, mentions: [target] }); } catch { await sock.sendMessage(ctx.chatId, { text: `❌ Erro ao rebaixar.` }); }
  },
  'link': async (sock, ctx) => {
    if (!ctx.isGroup || !(await utils.hasGroupAdminRights(sock, ctx.chatId, ctx.senderId))) return;
    try { const code = await sock.groupInviteCode(ctx.chatId); await sock.sendMessage(ctx.chatId, { text: `🔗 https://chat.whatsapp.com/${code}` }); } catch { await sock.sendMessage(ctx.chatId, { text: `❌ Erro ao obter link.` }); }
  },
  'dlt': async (sock, ctx) => {
    if (!ctx.isGroup || !(await utils.hasGroupAdminRights(sock, ctx.chatId, ctx.senderId))) return;
    ...
    try { await sock.sendMessage(ctx.chatId, { delete: { remoteJid: ctx.chatId, id: quoted.stanzaId, participant: quoted.participant } }); await utils.reagir(sock, ctx.msg, '✅'); } catch { await sock.sendMessage(ctx.chatId, { text: `❌ Não consegui apagar.` }); }
  },
  'rg': async (sock, ctx) => {
  ...
  'antil': async (sock, ctx) => {
    ...
    if (sub === 'add') { const d = ctx.args[1]; if (!d) return; const host = d.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0]; if (!db.whitelist.has(ctx.chatId)) db.whitelist.set(ctx.chatId, new Set()); db.whitelist.get(ctx.chatId).add(host); salvarDados(); return sock.sendMessage(ctx.chatId, { text: `✅ ${host} adicionado` }); }
    if (sub === 'remove') { const d = ctx.args[1]; if (!d) return; const host = d.replace(/^https?:\/\//, '').replace(/^www\./, '').split('/')[0]; const s = db.whitelist.get(ctx.chatId); if (s && s.has(host)) { s.delete(host); salvarDados(); return sock.sendMessage(ctx.chatId, { text: `✅ ${host} removido` }); } return sock.sendMessage(ctx.chatId, { text: `️ Não estava na whitelist` }); }
    if (sub === 'ls' || sub === 'list') { const s = db.whitelist.get(ctx.chatId) || new Set(); if (!s.size) return sock.sendMessage(ctx.chatId, { text: '📝 Whitelist vazia.' }); return sock.sendMessage(ctx.chatId, { text: `Whitelist:\n${[...s].join('\n')}` }); }
    if (['ban', 'kick', 'delete', 'warn'].includes(sub)) { db.grupos.antiLink.set(ctx.chatId, sub); salvarDados(); return sock.sendMessage(ctx.chatId, { text: `🔗 Anti-link: ${sub}` }); }
  ...
  'listb': async (sock, ctx) => {
    ...
    await sock.sendMessage(ctx.chatId, { text: '🎵 *TikTok Download*\n\n⏳ Aguarda um momentinho...\n🔍 A encontrar o vídeo...' });
    try {
  ...
  'yt': async (sock, ctx) => {
    ...
    try {
      ...
      if (thumbnail) {
        await sock.sendMessage(ctx.chatId, { image: { url: thumbnail }, caption: texto });
      } else {
        await sock.sendMessage(ctx.chatId, { text: texto });
      }
    ...
    try {
      ...
      if (buf) {
        ...
        return await sock.sendMessage(ctx.chatId, {
          ...
          fileName: `${(dados.title || 'audio').replace(/[^a-z0-9]/gi, '_').substring(0, 50)}.${ehMp3 ? 'mp3' : 'm4a'}`,
          ptt: false,
  ...
  'stm': async (sock, ctx) => {
    ...
    try {
      ...
      else if (quotedMsg.documentMessage) { buffer = await downloadMediaMessage(mediaMsg, 'buffer', {}); tipo = 'document'; }
      // Se a palavra-chave começa com ".", é um comando/menu
      if (palavra.startsWith('.')) {
        ...
        salvarDados();
        await sock.sendMessage(ctx.chatId, { text: `✅ *Mídia salva!*\n\n🔑 Chave: *${palavra}*\n📁 Tipo: ${tipo}\n\nQuando alguém disser "${palavra}", o bot enviará esta mídia.` });
      }
...
rlTerminal.on('line', (linha) => {
  ...
  else if (cmd === 'reiniciar' || cmd === '.reiniciar') { mensagensIgnoradas = new Set(ultimasMensagensIds); console.log(`🔄 A reiniciar — ${ultimasMensagensIds.length} mensagens serão ignoradas.`); try { sockAtual?.end(new Error('Reinício manual')); } catch {} }
...
async function startBot() {
  ...
  try {
    ...
    // ✅ Limpa históricos de IA inativos a cada 30 min
    setInterval(() => {
      try {
        const AGORA = Date.now();
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        senderId = await resolverIdDono(sock, chatId, senderId); // ✅ FIX DONO (LID)
        ...
        // ✅ PV: Apenas o dono ou utilizadores VIP podem interagir livremente.
        ...
        if (isGroup && fullText) {
          ...
          if (dadosTransacoes?.ativo) {
            ...
            if (comprovativo) {
              ...
              if (contaValida) {
                ...
                dadosTransacoes.pedidos[numero] = { id: numero, usuario: senderId, valor: comprovativo.valor, data: new Date().toISOString(), status: 'pendente', idTransacao: comprovativo.idTransacao, tipo: comprovativo.tipo };
                salvarDados(); registrarAcao(chatId, `Pedido #${numero} registado (${comprovativo.tipo})`);
                await sock.sendMessage(chatId, { text: `🔄 PEDIDO #${numero} REGISTADO\n⏳ Aguarde revisão\n ${comprovativo.idTransacao} | 💰 ${comprovativo.valor} MT`, mentions: [senderId] });
                try {
                  const metadata = await sock.groupMetadata(chatId);
                  const admins = metadata.participants.filter(p => p.admin).map(p => p.id);
                  for (const adminId of [...admins, `${CONFIG.ownerId}@s.whatsapp.net`]) {
                    await sock.sendMessage(adminId, { text: `🔔 PEDIDO #${numero}\n👤 @${senderId.split('@')[0]} | 💰 ${comprovativo.valor} MT\n✅ .act ${numero} | ❌ .rj ${numero}`, mentions: [senderId] }).catch(() => {});
                  }
                } catch {}
                return;
              }
          ...
          if (!fullText.startsWith(CONFIG.prefix)) {
            ...
            if (db.grupos.audioGatilhos.has(chatId)) {
              const audioMap = db.grupos.audioGatilhos.get(chatId);
              for (const [palavra, audioBuffer] of audioMap) {
                if (fullText.toLowerCase().includes(palavra)) {
            ...
            if (db.grupos.midiaGatilhos.has(chatId)) {
              const midiaMap = db.grupos.midiaGatilhos.get(chatId);
              for (const [palavra, {buffer, tipo}] of midiaMap) {
                if (fullText.toLowerCase().includes(palavra)) {
                  ...
                  else if (tipo === 'document') await sock.sendMessage(chatId, { document: buffer, mimetype: 'application/octet-stream', fileName: `${palavra}.bin` });
                  return;
                }
              }
            }
            ...
            if (textoLower.includes('nano') || textoLower.includes('bot') || textoLower.includes('@' + CONFIG.botNumber)) {
              ...
              if (soChamouOBot) {
                ...
                if (!limiteMenu.permitido) { const seg = Math.ceil(limiteMenu.esperarMs / 1000); const tempo = seg > 60 ? `${Math.ceil(seg / 60)} min` : `${seg}s`; await sock.sendMessage(chatId, { text: `⏳ Aguarda ${tempo}` }); return; }
                await enviarMenuComBotoes(sock, chatId, senderId);
                return;
              }
              if (utils.isOwner(senderId) && isGroup && pareceIntentoSairGrupo(fullText)) { await sock.sendMessage(chatId, { text: '👋 Até já!' }); setTimeout(() => sock.groupLeave(chatId), 2000); return; }
              if (isGroup && pareceIntentoQuemDono(fullText)) { await sock.sendMessage(chatId, { text: `👤 Fui criado por *${CONFIG.creator}*.\n📞 ${CONFIG.ownerNumber}` }); return; }
              ...
              if (!limiteChat.permitido) { const seg = Math.ceil(limiteChat.esperarMs / 1000); const tempo = seg > 60 ? `${Math.ceil(seg / 60)} min` : `${seg}s`; await sock.sendMessage(chatId, { text: `⏳ Aguarda ${tempo}` }); return; }
            ...
            if (db.grupos.iaAtivo.has(chatId) && fullText.length > 2) {
              ...
              if (!limiteChatLivre.permitido) { const seg = Math.ceil(limiteChatLivre.esperarMs / 1000); const tempo = seg > 60 ? `${Math.ceil(seg / 60)} min` : `${seg}s`; await sock.sendMessage(chatId, { text: `⏳ Aguarda ${tempo}`, quoted: msg }); return; }
              const resposta = await askGroq(chatId, fullText, utils.isOwner(senderId), true);
        ...
        if (fullText?.startsWith(CONFIG.prefix)) {
          ...
          if (cmd && commands[cmd]) {
            // ✅ Verifica se o comando está desativado neste grupo
            ...
            if (MENU_COMANDOS.has(cmd)) {
              ...
              if (!limiteMenu.permitido) { const seg = Math.ceil(limiteMenu.esperarMs / 1000); const tempo = seg > 60 ? `${Math.ceil(seg / 60)} min` : `${seg}s`; return await sock.sendMessage(chatId, { text: `⏳ Aguarda ${tempo}` }); }
            }
            const rl = verificarRateLimit(senderId, cmd);
            if (!rl.permitido) { const seg = Math.ceil(rl.esperarMs / 1000); const tempo = seg > 60 ? `${Math.ceil(seg / 60)} min` : `${seg}s`; return await sock.sendMessage(chatId, { text: `⏳ Aguarda ${tempo}` }); }
            try { const cur = db.stats.get(cmd) || 0; db.stats.set(cmd, cur + 1); salvarDados(); } catch {}
            try {
              // ✅ Envia imagem do comando se existir (ex: .menu, .cgeral)
              const midiaComando = db.grupos.midiaComandos.get(chatId)?.get(`.${cmd}`);
              if (midiaComando) {
                if (midiaComando.tipo === 'image') await sock.sendMessage(chatId, { image: midiaComando.buffer, caption: '' });
                else if (midiaComando.tipo === 'video') await sock.sendMessage(chatId, { video: midiaComando.buffer, mimetype: 'video/mp4', caption: '' });
...
if (!process.env.GROQ_API_KEY) {
  console.warn('⚠️ GROQ_API_KEY não está definida no .env');
}
