...
const CONFIG = {
  botName: "Nano Bot 🤖",
  creator: "Yanik Uaite",
  ownerId: "275381038891241", // ✅ LID real do dono
...
// 🔑 CHAVE GROQ DIRECTAMENTE NO CÓDIGO (substitui pela tua chave real gsk_...)
const GROQ_API_KEY_DIRETA = "COLE_AQUI_A_TUA_CHAVE";

const groq = new Groq({
  apiKey: process.env.GROQ_API_KEY || GROQ_API_KEY_DIRETA
});
...
const db = {
  ...
  usersVIP: new Map(), // ✅ VIP de utilizadores individuais
  grupos: {
    antiLink: new Map(),
...
// ✅ Cache de groupMetadata (TTL 30s)
...
// ✅ FIX DONO: cache de LIDs já validados
...
// ✅ Hierarquia de VIP para UTILIZADORES
...
// ✅ Verifica VIP de utilizador para downloads
...
}// =================== IA (fallback automático de modelos) ===================
...
// ✅ Cartão de apresentação VIVO (saudação aleatória)
function gerarCartaoApresentacao() {
  ...
  return `╔══════════════════════════════╗\n║     💚 NANO BOT 🤖           ║\n║     _Assistente Inteligente_   ║\n╚══════════════════════════════╝\n\n${saudacao}\n\n📌 *QUEM SOU EU?*\nSou um assistente pessoal de WhatsApp\ncriado por *${CONFIG.creator}* para\ntornar a tua vida mais fácil! 🚀\n\n✨ *O QUE EU FAÇO:*\n┃ 🛡️ Protejo e gerencio grupos\n┃ 💎 Sistema VIP exclusivo\n┃ 💰 Pagamentos E-Mola/M-Pesa\n┃ 🎵 Downloads (TikTok, IG, YT)\n┃ 🌍 Tradutor de idiomas\n┃ 🧠 Inteligência Artificial\n┃ 🎨 Criador de stickers\n┃ 🛒 Loja Yanikzx9 Store\n\n📞 *QUER SABER MAIS?*\n✆ ${CONFIG.ownerNumber}\n📧 yanikuaite@gmail.com\n\n💬 _"Tecnologia ao serviço da_\n_tua comunidade!"_\n\n◈━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◈\n     💚 ${CONFIG.botName} © 2026\n◈━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◈`;
}
...
const commands = {
  ...
  'cadmin': async (sock, ctx) => {
    ...
    const botoes = [];
    if (p.pAdmin) {
      texto += `\n  ╠▸ .all [msg]  └─ Marcar todos\n  ╠▸ .close      └─ Fechar grupo\n  ╠▸ .open       └─ Abrir grupo\n  ╠▸ .link       └─ Link convite\n  ╠▸ .tid        └─ ID do grupo\n  ╠▸ .dlt        └─ Apagar msg\n  ╠▸ .dst [cmd]  └─ Desativar cmd\n  ╠▸ .actcmd [cmd] └─ Ativar cmd\n  ╠▸ .listad     └─ Cmds desativados\n`;
  ...
  'cmidia': async (sock, ctx) => {
    const corpo = `╔══════════════════════════════╗\n  ◈ N A N O B O T 🤖 ◈\n╚══════════════════════════════╝\n\n  ➤ 📲 *MÍDIA & UTILITÁRIOS*\n  ─────────────────────\n\n  ╠▸ .traduzir [texto]\n  ╠▸ .tr [texto]\n  ╠▸ .tk [link]       └─ TikTok\n  ╠▸ .ig [link]       └─ Instagram\n  ╠▸ .yt [pesquisa]   └─ YouTube\n  ╠▸ .ytd [link]      └─ YT áudio\n  ╠▸ .ytv [link]      └─ YT vídeo\n  ╠▸ .sta [palavra]   └─ Salvar áudio\n  ╠▸ .stm [palavra]   └─ Salvar mídia\n  ╚▸ .grcb [nível] [dias] [nº]\n\n◈━━━━━━━━━━━━━━━━━━━━━━━━━━━━━◈`;
  ...
  'info': async (sock, ctx) => {
    ...
    const frases = [
      ...
      '_"Protegendo e servindo os teus grupos!"_ 🛡️'
    ];
    const frase = frases[Math.floor(Math.random() * frases.length)];
  ...
  'antil': async (sock, ctx) => {
    ...
    if (['ban', 'kick', 'delete', 'warn'].includes(sub)) {
      ...
      const modos = { ban: '🔨 Banir quem enviar', kick: '👢 Expulsar quem enviar', delete: '🗑️ Apagar o link silenciosamente', warn: '⚠️ Avisar e apagar' };
      return sock.sendMessage(ctx.chatId, { text: `╭━━━━━━━━━━━━━━━━━━━━━━━╮\n 🛡️ *ANTI-LINK ACTIVADO!*\n┃\n┃ 📋 Modo: *${sub.toUpperCase()}*\n┃ ${modos[sub]}\n┃\n┃ ⚠️ _Links não autorizados_\n┃ _serão tratados!_\n╰━━━━━━━━━━━━━━━━━━━━━━━╯` });
    }
  ...
  'ytv': async (sock, ctx) => {
    ...
    await sock.sendMessage(ctx.chatId, { text: '😔 *Ops! Algo correu mal...*\n\n❌ Não consegui baixar o vídeo.\n\n💡 *Dicas:*\n├─ Verifica o link\n├─ Vídeos > 30 min não são suportados\n└─ Tenta novamente mais tarde' });
  },'vp': async (sock, ctx) => {
    if (!utils.isOwner(ctx.senderId)) throw new PermissaoNegada();
    let target = utils.getQuotedMention(ctx.msg) || utils.getMentions(ctx.msg)[0];
    if (!target && ctx.args[0]) target = `${ctx.args[0].replace(/\D/g, '')}@s.whatsapp.net`;
      ...
      try {
        ...
        if (isGroup && fullText) {
          ...
          if (!fullText.startsWith(CONFIG.prefix)) {
            ...
            // ✅ Saudações VIVAS (reação + resposta aleatória)
            if (textoLower === 'bom dia') {
              await utils.reagir(sock, msg, '☀️');
              const respostas = ['Bom dia! Que o dia seja incrível! ☀️', 'Bom diaaa! 🌅 Energia positiva pra ti!', 'Bom dia, craque! Vamos nessa! 💪'];
              ...
              const respostas = ['Boa tarde! Tá correndo tudo bem? 😊', 'Boa tarde! Meio do dia e a energia não para! 🔥', 'Boa tarde, chefe! 🌤️'];
              await sock.sendMessage(chatId, { text: respostas[Math.floor(Math.random() * respostas.length)] });
            }
            else if (textoLower === 'boa noite') {
              await utils.reagir(sock, msg, '🌙');
              const respostas = ['Boa noite! Descansa bem! 🌙', 'Boa noite! Bons sonhos! ✨', 'Boa noite! Amanhã é outro dia! 😴'];
              await sock.sendMessage(chatId, { text: respostas[Math.floor(Math.random() * respostas.length)] });
            }
            else if (textoLower.includes('obrigado') || textoLower.includes('obrigada') || textoLower.includes('valeu')) {
              await utils.reagir(sock, msg, '💚');
              const respostas = ['De nada! Tô aqui pra isso! 😊', 'Sempre às ordens! 💚', 'Por nada, chefe! Precisando é só chamar! 🤝'];
              await sock.sendMessage(chatId, { text: respostas[Math.floor(Math.random() * respostas.length)] });
            }
          }
        }
